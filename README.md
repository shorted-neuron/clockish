...
Run the dashboard on the Pi (the display starts updating once per second):

```bash
clockish configs/clockish.yaml
# or equivalently
python3 -m clockish configs/clockish.yaml
```

Pass `--debug` to print per-frame timing, or `--debug-layout` to render one frame and exit:

```bash
clockish --debug-layout configs/big-red.yaml
```
